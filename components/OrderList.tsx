...
export const OrderList: React.FC<OrderListProps> = ({ orders, onUpdateStatus }) => {
  ...
  const handleStatusChange = async (e: React.ChangeEvent<HTMLSelectElement>, order: Order) => {
    ...
    if (config?.botToken && targetUser) {
        ...
        const message = `سلام ${order.customerName}، وضعیت سفارش شما با شماره ${order.id} به ${statusLabel} تغییر کرد.`;
        
        const sent = await sendTextMessage(config.botToken, targetUser.userId, message);
        if (sent) {
            console.log('Telegram notification sent to', targetUser.firstName);
        } else {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6">
        {orders.map((order) => (
          <div key={order.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-colors hover:shadow-md">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
              <div>
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  <span className="ml-3">📱 {order.customerPhone || '---'}</span>
              ...
              <div className="flex flex-col items-end gap-2 w-full md:w-auto">
                ...
                <select
                  ...
                  className="text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-2 py-1 bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200 outline-none focus:ring-2 focus:ring-brand-500"
                >
            ...
            <div className="border-t border-gray-100 dark:border-gray-700 pt-4 mt-4">
              <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 mb-3">اقلام سفارش:</h4>
              <div className="space-y-2">
                {order.items.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center text-sm bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                    <div className="flex items-center gap-2">
                        <span className="w-6 h-6 bg-white dark:bg-gray-600 rounded-full flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-300 shadow-sm">
                            {item.quantity}
                        </span>
